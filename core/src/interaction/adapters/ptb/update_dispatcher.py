...
from core.src.interaction.adapters.ptb.state_store import PtbUserDataStateStore
    ...
    async def handle_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        if update.callback_query is not None:
            try:
                ...
                # Not critical; we still continue processing.
                pass

        async with self._session_provider.session_scope() as session:
            messenger: Messenger = PtbMessenger(context.bot)
            store: StateStore = PtbUserDataStateStore(context.user_data)
            state = InteractionState(store)

            user_input = UserInput(update, context, session, messenger=messenger, state=state)
