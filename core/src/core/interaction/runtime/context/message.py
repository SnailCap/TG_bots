...
from .enums import UserInputType
...
class MessageInput:
    ...
    @staticmethod
    def from_update(update: Update) -> "MessageInput":
        ...
        if update.message is not None:
            ...
            if not raw_text:
                return MessageInput(type=UserInputType.MESSAGE, text=None, callback_data=None)

            if raw_text.startswith("/"):
                return MessageInput(type=UserInputType.COMMAND, text=raw_text[1:], callback_data=None)

            return MessageInput(type=UserInputType.MESSAGE, text=raw_text, callback_data=None)

        return MessageInput(type=UserInputType.UNKNOWN, text=None, callback_data=None)