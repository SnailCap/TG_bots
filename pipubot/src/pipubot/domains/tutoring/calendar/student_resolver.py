...
def _pick_best_candidate(candidates: list[TutoringStudent]) -> StudentResolveResult:
    ...
    1) If exactly one live candidate -> pick it
    ...
    live = [c for c in candidates if c.student_state in _LIVE_STUDENT_STATES]
    if len(live) == 1:
        return StudentResolveResult(
            kind="matched",
            parsed_name=None,
            student_id=live[0].id,
...
async def resolve_student_id_by_event_title(
    ...
    *,
    ...
    - Prefers exactly one live student when multiple candidates exist.
