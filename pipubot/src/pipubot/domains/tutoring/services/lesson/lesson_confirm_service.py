...
def _resolve_duration_min(dto: ConfirmLessonDTO, lesson: TutoringLesson) -> int:
    ...
    if dto.actual_start_at and dto.actual_end_at:
        try:
            return _duration_minutes_floor(dto.actual_start_at, dto.actual_end_at)
...
async def list_lessons_in_time_window(
    session: AsyncSession,
    *,
    tutor_user_id: int,
    start_from: datetime,
    ...
    stmt = stmt.order_by(TutoringLesson.start_at.asc(), TutoringLesson.id.asc()).limit(limit)
